...
namespace TradeMap.Service.Helpers
{
    public static class PageHelper<T> where T : class
    {
        ...
        public static List<T> Sorting(SortType.SortOrder sortType, IEnumerable<T> searchResult, string colName)
        {
            ...
            {
                return searchResult.OrderBy(item => typeof(T).GetProperties().First(x => x.Name.Contains(colName, System.StringComparison.CurrentCultureIgnoreCase)).GetValue(item)).ToList();
            }
            else if (sortType == SortType.SortOrder.Descending)
            {
                return searchResult.OrderByDescending(item => typeof(T).GetProperties().First(x => x.Name.Contains(colName, System.StringComparison.CurrentCultureIgnoreCase)).GetValue(item)).ToList();
            }
            else
            {
